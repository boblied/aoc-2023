...
my $MaxEnergized = 0;
for my $row ( 0 .. $Layout->height() )
{
    my $e = shine($row, 0, ">");
    $MaxEnergized = $e if $e > $MaxEnergized;
    $e = shine($row, $Layout->width(), "<");
    $MaxEnergized = $e if $e > $MaxEnergized;
}
for my $col ( 0 .. $Layout->width() )
{
    my $e = shine(0, $col, "v");
    $MaxEnergized = $e if $e > $MaxEnergized;
    $e = shine($Layout->height(), $col, "^");
    $MaxEnergized = $e if $e > $MaxEnergized;
}
say $MaxEnergized;
...
{
    @BeamStack = ( [ @startBeam ] );
    $Cave = AOC::Grid::makeGrid($Layout->height, $Layout->width, ".");
